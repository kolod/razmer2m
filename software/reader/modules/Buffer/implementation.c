...
static result_t writeAll_DISABLED(Buffer *self, uint8_t v);
static result_t writeAll_ENABLED(Buffer *self, uint8_t v);
...
void buffer_begin(Buffer *self){
	...
	self->writeAll = writeAll_DISABLED;
...
static result_t writeAll_ENABLED(Buffer *self, uint8_t v){
	return bufferBlock_writeAll(self->wblock, v);
}

result_t buffer_writeAll(Buffer *self, uint8_t v){
	return self->writeAll(self, v);
}
static result_t readNext_DISABLED(Buffer *self, uint8_t *out){
	return RESULT_FAILURE;
}
...
static result_t lockForWriting_ENABLED(Buffer *self){
	...
	self->writeAll = writeAll_ENABLED;
...
size_t buffer_getBlockLength(Buffer *self){
	return bufferBlock_getLength(&self->blocks[0]);
}
