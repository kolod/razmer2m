...
static result_t writeAll_DUMMY(BufferBlock *self, uint8_t v);
static result_t writeAll_ACTIVE(BufferBlock *self, uint8_t v);
...
result_t bufferBlock_writeAll(BufferBlock *self, uint8_t v){
...
size_t bufferBlock_getLength(BufferBlock *self){
	return BUFFER_BLOCK_SYMBOL_COUNT;
}
