...
stime_t usToStime (long int us) {
	...
	out.tv_sec = us / MICROSECONDS_IN_SECOND;
	out.tv_nsec = (us % MICROSECONDS_IN_SECOND) * NANOSECONDS_IN_MICROSECOND;
	return out;
}

stime_t msToStime (long int ms) {
	stime_t out = {0, 0};
	if (ms <= 0) {
		return out;
	}
	out.tv_sec = ms / MILLISECONDS_IN_SECOND;
	out.tv_nsec = (ms % MILLISECONDS_IN_SECOND) * NANOSECONDS_IN_MILLISECOND;
	return out;
}
...
unsigned long stimeToMs(stime_t v){
	unsigned long ms_in_s = v.tv_sec * MILLISECONDS_IN_SECOND;
	unsigned long ms_in_ns = v.tv_nsec / NANOSECONDS_IN_MILLISECOND;
	return ms_in_s + ms_in_ns;
}

unsigned long stimeToUs(stime_t v){
	unsigned long us_in_s = v.tv_sec * MICROSECONDS_IN_SECOND;
	unsigned long us_in_ns = v.tv_nsec / NANOSECONDS_IN_MICROSECOND;
	return us_in_s + us_in_ns;
}
...
void stime_delayIdleRest(stime_t interval, stime_t start) {
    stime_t end;
